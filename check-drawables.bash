...
SCRIPT=`basename $0`
cat << EOF
usage: $SCRIPT options [PROJECT]

This script checks each drawable in PROJECT/res/drawable (and related
folders) and searches all xml and java source files to determine if the
drawable is used in the project.
...
DRAWABLES=$(ls $PROJECT/res/drawable* -1 | sed 's/\..\+//g' | sort | uniq)
...
do
	...
	if grep -q @drawable/$file $XML_FILES
	then
		found=1
	fi
	if [ $found -eq 0 ]
	then
		...
		if grep -q R.drawable.$file $JAVA_FILES
		then
			found=1
		fi
	fi
